    ...
    try:
        ...
        if not keywords_with_scores:
            ...
            async with get_session() as session:
                db_keywords = await get_hot_keywords_from_db(session)
...
async def save_hot_keywords_controller():
    # session = get_session()
    async with get_session() as session:
        try:
            ...
            # 將熱門關鍵字和分數寫入 MySQL
            success = await save_hot_keywords_to_db(session, keywords_with_scores)
            if success:
